...
public class PartitionStatement extends StatementProcessor {
	...
	private boolean processPartitionProcedure(DDLStatement ddlStatement, DdlProceduresToLoad whichProcs)
			throws VoltCompilerException {
		...
		if (!statementMatcher.matches()) {
			throw m_compiler.new VoltCompilerException(String.format(
					"Invalid PARTITION statement: \"%s\", " + "expected syntax: PARTITION PROCEDURE <procedure> ON "
							+ "TABLE <table> COLUMN <column> [PARAMETER <parameter-index-no>]",
					ddlStatement.statement.substring(0, ddlStatement.statement.length() - 1))); // remove trailing
																								// semicolon
		}
		...
		m_tracker.addProcedurePartitionInfoTo(checkIdentifierStart(statementMatcher.group(1), ddlStatement.statement),
				partitionData);

		// this command is now deprecated as of VoltDB 7.0
		m_compiler.addWarn(
				"The standalone \"PARTITION PROCEDURE ...\" statement is deprecated. "
