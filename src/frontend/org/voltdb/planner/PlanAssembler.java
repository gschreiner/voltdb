...
public class PlanAssembler {
	// The convenience struct to accumulate results after parsing multiple
	// statements
	...
	private void setupForNewPlans(AbstractParsedStmt parsedStmt) {
		...
		if (parsedStmt instanceof ParsedSelectStmt) {
			...
			// Process the GROUP BY information, decide whether it is group by the partition
			// column
		...
		if (targetTable.getIsreplicated()) {
			...
			if (targetTable.getVerticalpartitioncolumns() != null
					&& !targetTable.getVerticalpartitioncolumns().isEmpty()) {
				Set<Column> cols = new HashSet<>();
				targetTable.getVerticalpartitioncolumns().forEach(col -> cols.add(col));
				m_partitioning.setVerticalPartitioningColumnForDML(cols);
			}
		}

		if (parsedStmt instanceof ParsedInsertStmt) {
			m_parsedInsert = (ParsedInsertStmt) parsedStmt;
			// The currently handled inserts are too simple to even require a subplan
			// assembler. So, done.
			return;
		}
		...
		if (!m_partitioning.wasSpecifiedAsSingle()) {
			...
			// misleadingly organizes the placement of join/where filters on the statement
			// tree.
			...
			HashMap<AbstractExpression, Set<AbstractExpression>> valueEquivalence = parsedStmt
	...
	private static void failIfNonDeterministicDml(AbstractParsedStmt parsedStmt, CompiledPlan plan) {
		...
		// This can happen if:
		// INSERT INTO ... SELECT ... where the select statement has a limit on
		// unordered data.
		...
		// Update doesn't have this issue yet (but having ORDER BY and LIMIT there
		// doesn't seem out
		// of the question).
		// When subqueries in WHERE clauses of DML are allowed, we will need to make
		// sure the
		...
		if (parsedStmt instanceof ParsedInsertStmt && !(plan.isOrderDeterministic() && contentDeterministic)) {
			...
			if (parsedStmt.m_isUpsert) {
				throw new PlanningErrorException("UPSERT statement manipulates data in a non-deterministic way.  "
			...
			if (targetHasLimitRowsTrigger) {
				throw new PlanningErrorException(
						"Order of rows produced by SELECT statement in INSERT INTO ... SELECT is "
								...
								+ "to address this issue." + contentDeterministicMsg);
			}
			...
			if (!contentDeterministic) {
				throw new PlanningErrorException(
						"INSERT statement manipulates data in a non-deterministic way." + contentDeterministicMsg);
			}
		}
	 ...
	 *
	 * @param parsedStmt
	 *            Current SQL statement to generate plan for
	...
	CompiledPlan getBestCostPlan(AbstractParsedStmt parsedStmt) {
		...
		if (fromSubqueryResult != null) {
			// Calculate the combined state of determinism for the parent and child
			// statements
			...
			if (orderIsDeterministic && !fromSubqueryResult.m_orderIsDeterministic) {
				// TODO: this reliance on the vague
				// isOrderDeterministicInSpiteOfUnorderedSubqueries test
				// is subject to false negatives for determinism. It misses the subtlety of
				// parent
				// queries that surgically add orderings for specific "key" columns of a
				// subquery result
				// or a subquery-based join for an effectively deterministic result.
				// The first step towards repairing this would involve detecting deterministic
				// and
				// non-deterministic subquery results IN CONTEXT where they are scanned in the
				// parent
				...
				// that can be exploited to impose determinism with fewer parent order by
				// columns
				// -- like just the keys.
			...
			// Need to re-attach the sub-queries plans to the best parent plan. The same
			// best plan for each
			// sub-query is reused with all parent candidate plans and needs to be
			// reconnected with
		...
		/*
		 * Find out if the query is inherently content deterministic and remember it.
		 */
	...
	/**
	 * Generate best cost plans for a list of derived tables, which we call FROM
	 * sub-queries and common table queries.
	 *
	 * @param subqueryNodes
	 *            - list of FROM sub-queries.
	...
	private boolean getBestCostPlanForExpressionSubQueries(Set<AbstractExpression> subqueryExprs) {
		...
		for (AbstractExpression expr : subqueryExprs) {
			...
			// The subquery plan must not contain Receive/Send nodes because it will be
			// executed
	 ...
	 * This method gets called repeatedly until it returns null, meaning there are
	 * no more plans.
	 *
	 * @return A not-previously returned query plan or null if no more computable
	 *         plans.
	 */
	...
	/**
	 * This is a UNION specific method. Generate a unique and correct plan for the
	 * current SQL UNION statement by building the best plans for each individual
	...
	private CompiledPlan getNextUnionPlan() {
		...
		for (AbstractParsedStmt parsedChildStmt : m_parsedUnion.m_children) {
			...
			if (commonPartitioning.requiresTwoFragments()) {
				...
				// the new statement is apparently a replicated read and has no effect on
				// partitioning
				continue;
			}
	...
	private static PlanDisposer SubqueryDisposer = new PlanDisposer() {
	...
	private int planTableScan(StmtEphemeralTableScan scan, int planId, AbstractParsedStmt stmt, PlanDisposer disposer) {
		...
		// TODO: It may make more sense to plan ahead and not generate the send/receive
		// pair
		...
		if (scan.canRunInOneFragment()) {
			...
			// in the invalid access plan - the subquery result order is significant in this
			// case
	...
	/**
	 * For each sub-query or CTE node in the plan tree, attach the corresponding
	 * plans to the parent node.
	 *
	 * @param initial
	 *            plan
	 */
	...
	private CompiledPlan getNextSelectPlan() {
		...
		/*
		 * If the access plan for the table in the join order was for a distributed
		 * table scan there must be a send/receive pair at the top EXCEPT for the
		 * special outer join case in which a replicated table was on the OUTER side of
		 * an outer join across from the (joined) scan of the partitioned table(s) (all
		 * of them) in the query. In that case, the one required send/receive pair is
		 * already in the plan below the inner side of a NestLoop join.
		 */
		if (m_partitioning.requiresTwoFragments()) {
			...
			if (receivers.size() == 1) {
				...
				// scan a partitioned table except under the ReceivePlanNode that was just
				// found.
				...
				if (receivers.size() > 0) {
					throw new PlanningErrorException("This special case join between an outer replicated table and "
	...
	/**
	 * Return true if the plan referenced by root node needs a projection node
	 * appended to the top.
	 *
	 * This method does a lot of "if this node is an instance of this class.... else
	 * if this node is an instance of this other class..." Perhaps it could be
	 * replaced by a virtual method on AbstractPlanNode?
	 *
	 * @param root
	...
	private boolean needProjectionNode(AbstractPlanNode root) {
		...
		// If there is a complex aggregation case, the projection plan node is already
		// added
		// right above the group by plan node. In future, we may inline that projection
		// node.
	...
	private static boolean disableNestedLoopIndexJoinForInComparison(AbstractPlanNode root,
	...
	static private boolean deleteIsTruncate(ParsedDeleteStmt stmt, AbstractPlanNode plan) {
		...
		// Assume all index scans have filters in this context, so only consider seq
		// scans.
	...
	private CompiledPlan getNextDeletePlan() {
		...
		if (deleteIsTruncate(m_parsedDelete, subSelectRoot)) {
			...
			if (m_parsedDelete.orderByColumns().size() > 0 && !isSinglePartitionPlan
					&& !targetTable.getIsreplicated()) {
				throw new PlanningErrorException(
						"DELETE statements affecting partitioned tables must " + "be able to execute on one partition "
								+ "when ORDER BY and LIMIT or OFFSET clauses " + "are present.");
			}
			...
			proj_schema.addColumn(AbstractParsedStmt.TEMP_TABLE_NAME, AbstractParsedStmt.TEMP_TABLE_NAME,
					"tuple_address", "tuple_address", addressExpr);
			if (needsOrderByNode) {
	...
	/**
	 * Get the next (only) plan for a VoltDB SWAP TABLE statement. These are pretty
	 * simple and will only generate a single plan.
	 *
	...
	private CompiledPlan getNextUpdatePlan() {
		...
		// FIXME: does this assert need to be relaxed in the face of non-from-clause
		// subquery support?
		...
		proj_schema.addColumn(AbstractParsedStmt.TEMP_TABLE_NAME, AbstractParsedStmt.TEMP_TABLE_NAME, "tuple_address",
				"tuple_address", tae);
		...
		for (Entry<Column, AbstractExpression> colEntry : m_parsedUpdate.m_columns.entrySet()) {
			Column col = colEntry.getKey();
			String colName = col.getTypeName();
			AbstractExpression expr = colEntry.getValue();

			proj_schema.addColumn(AbstractParsedStmt.TEMP_TABLE_NAME, AbstractParsedStmt.TEMP_TABLE_NAME, colName,
					colName, expr);
	...
	static private AbstractExpression castExprIfNeeded(AbstractExpression expr, Column column) {
		if (expr.getValueType().getValue() != column.getType() || expr.getValueSize() != column.getSize()) {
			...
			// We don't really support parameterized casting, such as specifically to
			// "VARCHAR(3)"
			// vs. just VARCHAR, but set the size parameter anyway in this case to make sure
			// that
			// the tuple that gets the result of the cast can be properly formatted as
			// inline.
			// A too-wide value survives the cast (to generic VARCHAR of any length) but the
			// attempt to cache the result in the inline temp tuple storage will throw an
			// early
			// runtime error on be half of the target table column.
			// The important thing here is to leave the formatting hint in the output schema
			// that
	...
	/**
	 * Get the next (only) plan for a SQL insertion. Inserts are pretty simple and
	 * this will only generate a single plan.
	 *
	...
	private CompiledPlan getNextInsertPlan() {
		...
		if (m_partitioning.getVerticalPartitioningInsertPlan() != null) {
			CompiledPlan plan = m_partitioning.getVerticalPartitioningInsertPlan();
			m_partitioning.setVerticalPartitioningInsertPlan(null);
			return plan;
		}
		m_bestAndOnlyPlanWasGenerated = true;
		...
		if (subquery != null) {
			...
			if (subquery.getBestCostPlan() == null) {
				...
				throw new PlanningErrorException(
						"INSERT INTO ... SELECT subquery could not be planned: " + m_recentErrorMsg);

			}

			boolean targetIsExportTable = tableListIncludesExportOnly(m_parsedInsert.m_tableList);
			InsertSubPlanAssembler subPlanAssembler = new InsertSubPlanAssembler(m_catalogDb, m_parsedInsert,
					m_partitioning, targetIsExportTable);
			...
			for (Constraint constraint : targetTable.getConstraints()) {
				...
				for (ColumnRef colRef : constraint.getIndex().getColumns()) {
					...
					if (!targetsPrimaryKey) {
						throw new PlanningErrorException("UPSERT on table \"" + targetTable.getTypeName()
								+ "\" must specify a value for primary key \"" + colRef.getColumn().getTypeName()
								+ "\".");
					}
				}
			}
			if (!hasPrimaryKey) {
				throw new PlanningErrorException("UPSERT is not allowed on table \"" + targetTable.getTypeName()
		...
		// Is just a workaround, but we need consider that different columns canot be
		// null, and need prepare the code for that, now tha sistem just consider
		// important columns in primarykey list.

		String verticalPartitionColumns = "";
		int vertI = 0;
		for (Column col : targetTableColumns) {
			boolean needsValue = (!m_parsedInsert.m_isUpsert) && (col.getNullable() == false)
					&& (col.getDefaulttype() == 0);
			...
			if (m_partitioning.getVerticalPartitionColForDML() != null) {
				for (Column column : m_partitioning.getVerticalPartitionColForDML())
					containsCol = containsCol || column.getTypeName().equals(col.getTypeName());

				if (subquery == null && containsCol) {
					String fullColumnName = targetTable.getTypeName() + "." + col.getTypeName();
					if (verticalPartitionColumns.isEmpty()) {
						verticalPartitionColumns += fullColumnName;
					} else {
						verticalPartitionColumns += ";" + fullColumnName;
					}
					if (m_parsedInsert.m_columns.containsKey(col)) {
						vertI++;
					}
				}
			}
		...
		for (Map.Entry<Column, AbstractExpression> e : m_parsedInsert.m_columns.entrySet()) {
			...
			if (pkList != null) {
				for (ColumnRef c : pkList) {
					if (c.getTypeName().equals(col.getTypeName())) {
						isMandatory = true;
						break;
					}
				}
		...
		// Thinking in the Vertical Partition storage system we change this mehots, now
		// the root also can be some other structure to force the system send to
...
//		}
		...
		if (!m_partitioning.getVerticalPartitionColForDML().isEmpty()) {
			...
			// rootIns1.addInlinePlanNode(rootIns2);
			//
			// SendPlanNode recvNode = new SendPlanNode();
			// recvNode.addAndLinkChild(insertNode);
			////
			// SendPlanNode recvNodeCom = new SendPlanNode();
			// recvNodeCom.addAndLinkChild(insertNodeCom);
			//
			// recvNode.addInlinePlanNode(recvNodeCom);
			//
			// LimitPlanNode limitNode1 = new LimitPlanNode();
			// limitNode1.setLimit(1);
			// limitNode1.addAndLinkChild(recvNode);
			//
			// LimitPlanNode limitNode2 = new LimitPlanNode();
			// limitNode2.setLimit(1);
			// limitNode2.addAndLinkChild(recvNodeCom);
			// limitNode1.addInlinePlanNode(limitNode2);
			//
	...
	/**
	 * Add a receive node, a sum or limit node, and a send node to the given DML
	 * node. If the DML target is a replicated table, it will add a limit node,
	 * otherwise it adds a sum node.
	 *
	 * @param dmlRoot
	 * @param isReplicated
	 *            Whether or not the target table is a replicated table.
	 * @return
	 */
	private static AbstractPlanNode addCoordinatorToDMLNode(AbstractPlanNode dmlRoot, boolean isReplicated) {
		...
		if (isReplicated) {
			...
			// modify the same number of tuples in replicated table, so just pick the result
			// from
			// any partition.
			...
			TupleValueExpression count_tve = new TupleValueExpression(AbstractParsedStmt.TEMP_TABLE_NAME,
					AbstractParsedStmt.TEMP_TABLE_NAME, "modified_tuples", "modified_tuples", 0);
			...
			count_schema.addColumn(AbstractParsedStmt.TEMP_TABLE_NAME, AbstractParsedStmt.TEMP_TABLE_NAME,
					"modified_tuples", "modified_tuples", tve);
	 ...
	 * (filter) to it. If the root node can embed the projection do so. If not, add
	 * a new projection node.
	 *
	 * @param rootNode
	 *            The root of the plan-sub-graph to add the projection to.
	 * @return The new root of the plan-sub-graph (might be the same as the input).
	 */
	 ...
	 * statement has no ORDER BY clause, or if the subtree is already producing rows
	 * in the correct order. Note that a hash aggregate node will cause this to
	 * return true, and a serial or partial aggregate node may cause this to return
	 * true.
	 *
	 * @param parsedStmt
	 *            The statement whose plan may need an OrderByPlanNode
	 * @param root
	 *            The subtree which may need its output tuples ordered
	...
	private static boolean isOrderByNodeRequired(AbstractParsedStmt parsedStmt, AbstractPlanNode root) {
		...
		// Skip the explicit ORDER BY plan step if an IndexScan is already providing the
		// equivalent ordering.
		// Note that even tree index scans that produce values in their own "key order"
		// only report
		...
		// or window function clause. Even an intervening non-hash aggregate will not
		// interfere
		...
		// Again, we have to replace the TVEs for ORDER BY clause for these cases in
		// planning.
		//
		// Find the scan or join node.
		AbstractPlanNode probe;
		for (probe = root; !((probe instanceof AbstractJoinPlanNode) || (probe instanceof AbstractScanPlanNode))
				&& (probe != null); probe = (probe.getChildCount() > 0) ? probe.getChild(0) : null) {
	...
	/**
	 * Create an order by node as required by the statement and make it a parent of
	 * root.
	 *
	 * @param parsedStmt
	 *            Parsed statement, for context
	 * @param root
	 *            The root of the plan needing ordering
	 * @return new orderByNode (the new root) or the original root if no orderByNode
	 *         was required.
	 */
	...
	private AbstractPlanNode handleSelectLimitOperator(AbstractPlanNode root) {
		 ...
		 * , DISTINCT in aggregates and DISTINCT PUSH DOWN with partition column
		 * included.
		 */
		...
		/*
		 * Push down the limit plan node when possible even if offset is set. If the
		 * plan is for a partitioned table, do the push down. Otherwise, there is no
		 * need to do the push down work, the limit plan node will be run in the
		 * partition.
		 */
		if (canPushDown) {
			/*
			 * For partitioned table, the pushed-down limit plan node has a limit based on
			 * the combined limit and offset, which may require an expression if either of
			 * these was not a hard-coded constant and didn't get parameterized. The top
			 * level limit plan node remains the same, with the original limit and offset
			 * values.
			 */
	...
	/**
	 * Inline limit plan node can be applied with ORDER BY node and serial
	 * aggregation node
	 *
	 * @param pn
	 * @return
	...
	static private boolean isInlineLimitPlanNodePossible(AbstractPlanNode pn) {
		if (pn instanceof OrderByPlanNode || pn.getPlanNodeType() == PlanNodeType.AGGREGATE) {
			return true;
		}
		return false;
	}

	private AbstractPlanNode handleMVBasedMultiPartQuery(HashAggregatePlanNode reAggNode, AbstractPlanNode root,
	...
	/**
	 * For a seqscan feeding a GROUP BY, consider substituting an IndexScan that
	 * pre-sorts by the GROUP BY keys. If a candidate is already an indexscan,
	 * simply calculate GROUP BY column coverage
	 *
	 * @param candidate
	 * @param gbInfo
	 * @return true when planner can switch to index scan from a sequential scan,
	 *         and when the index scan has no parent plan node or the candidate is
	 *         already an indexscan and covers all or some GROUP BY columns
	 */
	...
	private AbstractPlanNode handleAggregationOperators(AbstractPlanNode root) {
		...
		if (m_parsedSelect.hasAggregateOrGroupby()) {
			...
			for (int outputColumnIndex = 0; outputColumnIndex < m_parsedSelect.m_aggResultColumns
					.size(); outputColumnIndex += 1) {
				...
				if (rootExpr instanceof AggregateExpression) {
					...
					TupleValueExpression tve = new TupleValueExpression(AbstractParsedStmt.TEMP_TABLE_NAME,
							AbstractParsedStmt.TEMP_TABLE_NAME, "", col.m_alias, rootExpr, outputColumnIndex);
					...
					schema_col = new SchemaColumn(AbstractParsedStmt.TEMP_TABLE_NAME,
							AbstractParsedStmt.TEMP_TABLE_NAME, "", col.m_alias, tve, outputColumnIndex);
					top_schema_col = new SchemaColumn(AbstractParsedStmt.TEMP_TABLE_NAME,
							AbstractParsedStmt.TEMP_TABLE_NAME, "", col.m_alias, tve, outputColumnIndex);

					/*
					 * Special case count(*), count(), sum(), min() and max() to push them down to
					 * each partition. It will do the push-down if the select columns only contains
					 * the listed aggregate operators and other group-by columns. If the select
					 * columns includes any other aggregates, it will not do the push-down. - nshi
					 */
					if (topAggNode != null) {
						ExpressionType top_expression_type = agg_expression_type;
						/*
						 * For count(*), count() and sum(), the pushed-down aggregate node doesn't
						 * change. An extra sum() aggregate node is added to the coordinator to sum up
						 * the numbers from all the partitions. The input schema and the output schema
						 * of the sum() aggregate node is the same as the output schema of the push-down
						 * aggregate node.
						 *
						 * If DISTINCT is specified, don't do push-down for count() and sum() when not
						 * group by partition column. An exception is the aggregation arguments are the
						...
						/*
						 * For min() and max(), the pushed-down aggregate node doesn't change. An extra
						 * aggregate node of the same type is added to the coordinator. The input schema
						 * and the output schema of the top aggregate node is the same as the output
						 * schema of the pushed-down aggregate node.
						 *
						 * APPROX_COUNT_DISTINCT can be similarly pushed down, but must be split into
						 * two different functions, which is done later, from pushDownAggregate().
						 */
						...
						if (topAggNode != null) {
							/*
							 * Input column of the top aggregate node is the output column of the push-down
							 * aggregate node
							 */
					...
					/*
					 * These columns are the pass through columns that are not being aggregated on.
					 * These are the ones from the SELECT list. They MUST already exist in the child
					 * node's output. Find them and add them to the aggregate's output.
					 */
					schema_col = new SchemaColumn(col.m_tableName, col.m_tableAlias, col.m_columnName, col.m_alias,
							col.m_expression, outputColumnIndex);
					...
					top_schema_col = new SchemaColumn(col.m_tableName, col.m_tableAlias, col.m_columnName, col.m_alias,
	...
	private void calculateIndexGroupByInfo(IndexScanPlanNode root, IndexGroupByInfo gbInfo) {
		...
		gbInfo.m_canBeFullySerialized = (gbInfo.m_coveredGroupByColumns.size() == m_parsedSelect.groupByColumns()
				.size());
	}
	...
	/**
	 * This function is called once it's been determined that we can push down an
	 * aggregation plan node.
	 *
	 * If an APPROX_COUNT_DISTINCT aggregate is distributed, then we need to convert
	 * the distributed aggregate function to VALS_TO_HYPERLOGLOG, and the
	 * coordinating aggregate function to HYPERLOGLOGS_TO_CARD.
	 *
	 * @param distNode
	 *            The aggregate node executed on each partition
	 * @param coordNode
	 *            The aggregate node executed on the coordinator
	 */
	...
	/**
	 * Push the given aggregate if the plan is distributed, then add the coordinator
	 * node on top of the send/receive pair. If the plan is not distributed, or
	 * coordNode is not provided, the distNode is added at the top of the plan.
	 *
	 * Note: this works in part because the push-down node is also an acceptable top
	 * level node if the plan is not distributed. This wouldn't be true if we
	 * started pushing down something like (sum, count) to calculate a distributed
	 * average. (We already do something like this for APPROX_COUNT_DISTINCT, which
	 * must be split into two different functions for the pushed-down case.)
	 *
	 * @param root
	 *            The root node
	 * @param distNode
	 ...
	 * @param coordNode
	 *            [may be null] The top node to put on top of the send/receive pair
	 *            after push-down. If this is null, no push-down will be performed.
	 * @return The new root node.
	 */
	private static AbstractPlanNode pushDownAggregate(AbstractPlanNode root, AggregatePlanNode distNode,
			AggregatePlanNode coordNode, ParsedSelectStmt selectStmt) {
		...
		/*
		 * Push this node down to partition if it's distributed. First remove the
		 * send/receive pair, add the node, then put the send/receive pair back on top
		 * of the node, followed by another top node at the coordinator.
		 */
	...
	private static AbstractPlanNode processComplexAggProjectionNode(ParsedSelectStmt selectStmt,
	 ...
	 *
	 * Return a mid-plan send node, if one exists and can host a distributed limit
	 * node. There is guaranteed to be at most a single receive/send pair. Abort the
	 * search if a node that a "limit" can't be pushed past is found before its
	 * receive node.
	 *
	 * Can only push past: * coordinatingAggregator: a distributed aggregator a copy
	 * of which has already been pushed down. Distributing a LIMIT to just above
	 * that aggregator is correct. (I've got some doubts that this is correct???
	 * --paul)
	 *
	 * * order by: if the plan requires a sort, getNextSelectPlan() will have
	 * already added an ORDER BY. A distributed LIMIT will be added above a copy of
	 * that ORDER BY node.
	 *
	 ...
	 *         Otherwise null -- when the plan is single-partition when its
	 *         "coordinator" part contains a push-blocking node type.
	 */
	protected AbstractPlanNode checkLimitPushDownViability(AbstractPlanNode root) {
		...
		while (!(receiveNode instanceof ReceivePlanNode)) {
			...
			if (!(receiveNode instanceof OrderByPlanNode) && !(receiveNode instanceof ProjectionPlanNode)
	...
	private static boolean isValidAggregateNodeForLimitPushdown(AbstractPlanNode aggregateNode,
		...
		if (parent instanceof OrderByPlanNode) {
			orderByNode = parent;
		} else if (parent instanceof ProjectionPlanNode && parent.getParentCount() > 0
	 ...
	 * aggregation/grouping. DISTINCT is basically rewritten with GROUP BY to
	 * benefit from all kinds of GROUP BY optimizations. Trivial case DISTINCT in a
	 * statement with no GROUP BY has been rewritten very early at query parsing
	 * time. In the non-trivial case, where an existing GROUP BY column is NOT in
	 * the select list, DISTINCT can be implemented via a final aggregation (never
	 * pushed down) added to the top of the plan.
	 *
	 * @param root
	 *            can be an aggregate plan node or projection plan node
	 * @return
	 */
	private AbstractPlanNode handleDistinctWithGroupby(AbstractPlanNode root) {
		...
		// Ticket: ENG-7360
		/*
		 * boolean pushedDown = false; boolean canPushdownDistinctAgg =
		 * m_parsedSelect.hasPartitionColumnInDistinctGroupby(); // // disable pushdown,
		 * DISTINCT push down turns out complex // canPushdownDistinctAgg = false;
		 *
		 * if (canPushdownDistinctAgg && !m_parsedSelect.m_mvFixInfo.needed()) {
		 * assert(m_parsedSelect.hasPartitionColumnInGroupby()); AbstractPlanNode
		 * receive = root;
		 *
		 * if (receive instanceof ReceivePlanNode) { // Temporarily strip send/receive
		 * pair AbstractPlanNode distNode = receive.getChild(0).getChild(0);
	...
	/**
	 * Get the unique set of names of all columns that are part of an index on the
	 * given table.
	 *
	 * @param table
	 *            The table to build the list of index-affected columns with.
	 * @return The set of column names affected by indexes with duplicates removed.
	 */
	 ...
	 * http://citeseerx.ist.psu.edu/viewdoc/summary?doi=10.1.1.43.2531 Outerjoin
	 * Simplification and Reordering for Query Optimization by Cesar A.
	 * Galindo-Legaria , Arnon Rosenthal Algorithm: Traverse the join tree top-down:
	 * For each join node n1 do: For each expression expr (join and where) at the
	 * node n1 For each join node n2 descended from n1 do: If expr rejects nulls
	 * introduced by n2 inner table, then - convert LEFT OUTER n2 to an INNER join.
	 * - convert FULL OUTER n2 to RIGHT OUTER join If expr rejects nulls introduced
	 * by n2 outer table, then - convert RIGHT OUTER n2 to an INNER join. - convert
	 * FULL OUTER n2 to LEFT OUTER join
	 */
	...
	/**
	 * Verify if an expression from the input list is NULL-rejecting for any of the
